...
echo "COMPILING CITRA EMULATOR"
if [ -f "org.citra.citra_emu.apk" ]
then
    echo "PATCHING CITRA EMULATOR"
    java -jar cli.jar -m integrations.apk -b patches.jar \
        ${patches[@]} \
        $EXPERIMENTAL \
        -a org.citra.citra_emu.apk -o output/citra.emulator.apk
else
    echo "NO BASE PACKAGE, SKIP COMPILING CITRA EMULATOR"
fi
